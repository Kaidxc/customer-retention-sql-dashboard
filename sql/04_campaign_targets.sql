...
eligible_targets AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
            ORDER BY monetary_score DESC, total_revenue DESC, frequency_score DESC, days_since_last_purchase DESC
        ) AS priority_rank
    FROM segmented
...
SELECT
    ...
    monetary_score,
    rfm_segment,
    'High historical value but inactive for '
        || days_since_last_purchase
        || ' days' AS target_reason
